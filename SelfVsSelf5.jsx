...
export default function SelfVsSelf() {
  ...
  const updateTaskDate = (id, newDate) => {
    if (!newDate) return;
    const ti = todayISO();
    setTasks(tasks.map(x => x.id === id ? { ...x, scheduledDate: newDate, scheduledFor: undefined, forTomorrow: newDate > ti } : x));
    setEditingDateTaskId(null);
  };
  // 予定タスクの日付を消して「期限なし」にする
  const clearTaskDate = (id) => {
    setTasks(tasks.map(x => x.id === id ? { ...x, scheduledDate: null, scheduledFor: undefined, forTomorrow: false, planBonus: 0, planBonusDate: null } : x));
    setEditingDateTaskId(null);
  };
                    ...
                    {dayTasks.map(task => {
                      ...
                      return (
                        <div key={task.id} className={`rounded-xl p-3 flex items-center gap-2 mb-2 border-2 ${fDone ? 'bg-zinc-900/60 border-zinc-800' : 'bg-gradient-to-br from-blue-900/40 to-cyan-900/30 border-blue-600/60'}`}>
                            ...
                            {editingDateTaskId === task.id && (
                              <div className="mt-2 flex items-center gap-1.5">
                                <input
                                  ...
                                  className="bg-zinc-900 border border-cyan-700 rounded px-2 py-1 text-[11px] text-white focus:border-cyan-400 outline-none"
                                  autoFocus
                                />
                                <button onClick={() => clearTaskDate(task.id)} className="text-[10px] px-2 py-1 rounded border border-zinc-600 text-zinc-300 hover:bg-zinc-700/40" title="期限なしに変更">期限なし</button>
                                <button onClick={() => setEditingDateTaskId(null)} className="text-[10px] px-2 py-1 rounded border border-zinc-700 text-zinc-400 hover:bg-zinc-700/40">閉じる</button>
                              </div>
                            )}
                          </div>
                          ...
                          <button onClick={() => setEditingDateTaskId(editingDateTaskId === task.id ? null : task.id)} className="w-9 h-9 rounded-lg border border-cyan-800 hover:border-cyan-500 hover:bg-cyan-500/10 flex items-center justify-center transition active:scale-90 flex-shrink-0" title="日付を変更">
